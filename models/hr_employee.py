...
class HrEmployeeBase(models.AbstractModel):
    ...
    current_leave_state = fields.Selection(compute='_compute_leave_status', string="Current Time Off Status",
        selection=[
            ('draft', 'New'),
            ('confirm', 'Waiting for Line Manager Approval'),## previous value= waiting approval
            ('refuse', 'Rejected'),##previous value Refused
            ('validate1', 'Waiting for HOU Approval'),##prev Waiting Second Approval
            ('validate', 'Waiting for HR Approval'),##prev Approved
            ('validated', 'Approved'),##prev Approved
            ('cancel', 'Cancelled')
        ])
    ...
    def _compute_leave_manager(self):
        for employee in self:
            ...
            ##elif not employee.leave_manager_id:
             ##   employee.leave_manager_id = employee.parent_id##previous value = False; changed to manager
