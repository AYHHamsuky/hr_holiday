        ...
        for holiday in self.filtered('employee_id'):
            ...
            if nholidays:
                raise ValidationError(
                    _('You can not set 2 leave that overlaps on the same day for the same employee.') + '\n- %s' % (holiday.display_name))
    ...
    def _check_double_validation_rules(self, employees, state):
        ...
        if state == 'validate1':
            ...
            if employees and not is_leave_user:
                raise AccessError(_('You cannot first approve a time off for %s, because you are not his leave manager', employees[0].name))
